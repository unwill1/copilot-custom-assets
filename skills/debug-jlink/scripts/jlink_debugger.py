...
def generate_gdb_script(mode: str, elf_path: str, gdb_port: int) -> str:
    ...
    if mode == "download-and-halt":
        ...
        lines.extend([
            ...
            "print/x *((uint32_t*)0xE000ED3C)",  # AFSR
            "print/x *((uint32_t*)0xE000EFA8)",  # ABFSR (Cortex-M7 only)
            "quit",
        ])
...
示例:
  %(prog)s --detect
  %(prog)s --elf build/Project.axf --device GD32H759IM
  %(prog)s --elf build/Project.axf --device GD32H759IM --mode attach-only
  %(prog)s --elf build/Project.axf --device GD32H759IM --mode crash-context --swo
        """,
    )
