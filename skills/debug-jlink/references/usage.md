...
python3 skills/debug-jlink/scripts/jlink_debugger.py \
  --elf build/Project.axf --device GD32H759IM

# 附着调试
python3 skills/debug-jlink/scripts/jlink_debugger.py \
  --elf build/Project.axf --device GD32H759IM --mode attach-only

# 崩溃现场排查
python3 skills/debug-jlink/scripts/jlink_debugger.py \
  --elf build/Project.axf --device GD32H759IM --mode crash-context
```
...
| `--elf` | 带符号的 ELF 文件路径 |
| `--device` | 目标芯片型号（如 GD32H759IM、GD32F407VG、STM32F407VG） |
