...
python3 skills/flash-jlink/scripts/jlink_flasher.py \
  --artifact build/Project.axf \
  --device GD32H759IM

# 烧录 BIN（需要指定基地址）
python3 skills/flash-jlink/scripts/jlink_flasher.py \
  --artifact build/Project.bin \
  --device GD32H759IM \
  --base-address 0x08000000

# RTT 日志捕获
python3 skills/flash-jlink/scripts/jlink_flasher.py \
  --rtt --device GD32H759IM --rtt-duration 30
```
...
```bash
python3 skills/flash-jlink/scripts/jlink_flasher.py \
  --rtt --device GD32H759IM --rtt-duration 60
```
...
| `--device` | 目标芯片型号（如 GD32H759IM、GD32F407VG、STM32F407VG），J-Link 必需 |
