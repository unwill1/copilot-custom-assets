...
def find_objcopy() -> str | None:
    configured_gcc = get_tool_path("arm-none-eabi-gcc")
    if configured_gcc:
        gcc_path = Path(configured_gcc)
        objcopy_name = "arm-none-eabi-objcopy.exe" if platform.system() == "Windows" else "arm-none-eabi-objcopy"
        candidate = gcc_path.with_name(objcopy_name)
...
def prepare_artifact_for_jlink(artifact_path: str, artifact_kind: str) -> tuple[str, str, Path | None]:
    artifact = Path(artifact_path)

    if artifact_kind == "elf":
        sibling_hex = artifact.with_suffix(".hex")
        if sibling_hex.is_file():
            return str(sibling_hex), "hex", None

        objcopy = find_objcopy()
        if not objcopy:
            raise RuntimeError("未找到 arm-none-eabi-objcopy，且未找到同名 HEX 文件")

        temp_hex = Path(tempfile.NamedTemporaryFile(suffix=".hex", delete=False).name)
        try:
            result = subprocess.run(
                [objcopy, "-O", "ihex", str(artifact), str(temp_hex)],
                capture_output=True, text=True, timeout=30,
            )
        except subprocess.TimeoutExpired as exc:
            temp_hex.unlink(missing_ok=True)
            raise RuntimeError("objcopy 转换 HEX 超时") from exc

        if result.returncode != 0 or not temp_hex.is_file() or temp_hex.stat().st_size == 0:
            temp_hex.unlink(missing_ok=True)
            details = (result.stderr or result.stdout or "未知错误").strip()
            raise RuntimeError(f"objcopy 转换 HEX 失败: {details}")

        return str(temp_hex), "hex", temp_hex

    return artifact_path, artifact_kind, None
...
def build_jlink_script(
    ...
    if artifact_kind == "bin" and base_address:
        lines.insert(5, f"verifybin {artifact_posix} {base_address}")
    return "\n".join(lines) + "\n"
...
def run_flash(jlink: str, script_content: str, verbose: bool) -> tuple[bool, list[str]]:
    ...
    if any(marker in combined_lower for marker in failure_markers):
        ...
        evidence.extend(line for line in last_lines if line.strip())
        ...
        elif "unknown device" in combined_lower or "unknown command" in combined_lower or "unknown / supported format" in combined_lower:
            evidence.insert(0, "failure_hint: project-config-error (设备名或固件格式无效)")
        else:
    ...
    if "downloading file" in combined_lower and any(marker in combined_lower for marker in success_markers):
        print("✅ 烧录成功，校验通过")
...
def run_rtt(
    ...
    rtt_addr: str | None = None, rtt_range: str | None = None,
) -> list[str]:
    # RTT 搜索范围默认值覆盖常见 Cortex-M SRAM 区域（含 GD32H7 的 0x24000000）
    default_range = "0x20000000 0x80000, 0x24000000 0x100000"
    with tempfile.NamedTemporaryFile(
        mode="w", suffix=".jlink", delete=False, encoding="utf-8",
    ) as f:
        ...
        if rtt_addr:
            f.write(f"exec SetRTTAddr {rtt_addr}\n")
        else:
            f.write("exec SetRTTAddr 0\n")
        f.write(f"exec SetRTTSearchRanges {rtt_range or default_range}\n")
        script_path = f.name
...
示例:
  ...
  %(prog)s --artifact build/Project.axf --device GD32H759IM
  %(prog)s --artifact build/Project.hex --device GD32F407VG --interface JTAG
  %(prog)s --artifact build/fw.bin --device GD32H759IM --base-address 0x08000000
  %(prog)s --rtt --device GD32H759IM
        """,
    )
    parser.add_argument("--detect", action="store_true", help="探测 J-Link 环境和已连接设备")
    parser.add_argument("--artifact", help="固件产物路径")
    parser.add_argument("--device", help="目标芯片型号（如 GD32H759IM、STM32F407VG）")
    ...
    parser.add_argument("--rtt-addr", help="RTT 控制块地址（十六进制，已知时可跳过搜索）")
    parser.add_argument("--rtt-range", help="RTT 搜索范围，如 '0x24000000 0xD0000'")
...
def main() -> int:
    ...
    if args.rtt:
        ...
        if not jlink:
            ...
            return 1
        rtt_output = run_rtt(
            jlink, args.device, args.interface, args.speed, args.rtt_duration,
            rtt_addr=args.rtt_addr, rtt_range=args.rtt_range,
        )
        print("\n📡 RTT 输出:")
    ...
    if not args.device:
        print("❌ J-Link 烧录需要 --device 参数（如 GD32H759IM、STM32F407VG）。")
    ...
    prepared_temp: Path | None = None
    try:
        flash_artifact_path, flash_kind, prepared_temp = prepare_artifact_for_jlink(artifact_path, kind)
    except RuntimeError as exc:
        print(f"❌ 无法为 J-Link 准备固件产物: {exc}")
        return 1

    if flash_artifact_path != artifact_path:
        print(f"🔁 J-Link 实际烧录文件: {flash_artifact_path} [{flash_kind.upper()}]")

    # BIN 需要基地址
    if flash_kind == "bin" and not args.base_address:
        print("❌ BIN 文件必须提供 --base-address（烧录基地址）。")
        result = FlashResult(
            status="blocked",
            summary="BIN 文件缺少烧录基地址",
            artifact_path=artifact_path,
            artifact_kind=flash_kind,
            failure_category="artifact-missing",
        )
        print_flash_report(result)
        if prepared_temp is not None:
            prepared_temp.unlink(missing_ok=True)
        return 1

    # 生成 J-Link 脚本
    script = build_jlink_script(
        device=args.device,
        artifact=flash_artifact_path,
        artifact_kind=flash_kind,
